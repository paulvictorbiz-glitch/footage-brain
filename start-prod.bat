...
REM  No Vite dev server needed. Open http://localhost:8765
REM ──────────────────────────────────────────────────────────────────────────

REM Always run from the script's own directory (handles double-click + shortcuts)
cd /d "%~dp0"
...
echo [2/2] Starting backend on http://localhost:8765 ...
echo       The UI will be available at http://localhost:8765
echo       Press Ctrl+C in the server window to stop.
echo.
start "Footage Brain" cmd /k "cd backend && call venv\Scripts\activate.bat && python -m uvicorn app.main:app --host 0.0.0.0 --port 8765"

REM Give the server a moment to start, then open the browser
timeout /t 3 /nobreak >nul
start "" "http://localhost:8765"

echo [OK] Server starting. Browser opening at http://localhost:8765
echo.
pause
