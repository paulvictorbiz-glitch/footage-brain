              ...
              {results.map((r) => (
                <div key={r.video_file_id} className="relative group/card">
                  ...
                  <VideoCard
                    ...
                    frameTimestamp={isVisualMode(filters.mode) ? r.frame_matches?.[0]?.timestamp : undefined}
                  />
                </div>
              ))}
            </div>
              ...
              {results.map((r) => (
                <div key={r.video_file_id}
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {isVisualMode(filters.mode) && r.frame_matches?.[0] ? (
                      <p className={cn('text-xs truncate mt-0.5',
                        filters.mode === 'visual' ? 'text-violet-400' :
                        filters.mode === 'caption' ? 'text-emerald-400' : 'text-amber-400')}>
                        {filters.mode === 'visual' ? 'Visual match' :
                          filters.mode === 'caption' ? 'Caption match' : 'Multimodal match'}
                        {' @ '}{r.frame_matches[0].timestamp.toFixed(0)}s
                      </p>
                    ) : r.matched_chunks[0] ? (
