...
function IndexingSpeedCard() {
  ...
  return (
    <div className="card p-4 space-y-3">
          ...
          return (
            <div key={s.stage}>
              <div className="flex items-center justify-between mb-1">
                ...
                <div className="flex items-center gap-2">
                  {s.rate_per_hour > 0 && !isSkipped && (
                    <span className="text-xs text-amber-400 font-mono">{s.rate_per_hour}/hr</span>
                  )}
      ...
      {data.eta_hours && (
        <div className="text-xs text-zinc-500 pt-1 border-t border-surface-4">
          {'ETA: ~'}{data.eta_hours < 1
            ? `${Math.round(data.eta_hours * 60)} minutes`
            : `${data.eta_hours} hours`}
          {' - '}{data.total_pending?.toLocaleString()} jobs pending
        </div>
      )}
    </div>
  )
