...
# ─────────────────────────────────────────────────────────────────────────────
# Caption search (VLM-generated frame captions)
# ─────────────────────────────────────────────────────────────────────────────
...
def caption_search(
    ...
    """
    Search the VLM caption stream. Captions live in the same sentence-transformer
    vector space as transcript chunks, so we reuse embed_text() to encode the
    query. Each hit is a captioned frame; we group by file and attach frame
    timestamps as FrameMatch entries so the UI can deep-link to the moment.
    """
    from app.ingest.embedder import embed_text
    ...
    try:
        ...
        logger.error("caption_embed_query_error", error=str(exc))
        return []
    ...
    hits = store.query(query_emb, n_results=n_results * 5)
    if not hits:
        return []
    ...
    for h in hits:
        ...
        if not fid:
            continue
        file_frames.setdefault(fid, []).append(h)
...
def _merge_results(streams: List[List[SearchResult]], n_results: int) -> List[SearchResult]:
    """
    Merge multiple ranked result lists by best score per file. Preserves
    chunk + frame matches from whichever stream contributed them so the UI
    can render all matched moments together.
    """
    ...
    for stream in streams:
        for r in stream:
            ...
            if existing is None:
                ...
                continue
            if r.best_score > existing.best_score:
                existing.best_score = r.best_score
            # accumulate matched chunks / frames from all streams
            if r.matched_chunks:
                existing.matched_chunks = (existing.matched_chunks or []) + r.matched_chunks
            if r.frame_matches:
                existing.frame_matches = (existing.frame_matches or []) + r.frame_matches
...
def search(
    ...
    if mode == "keyword":
        ...
        return _merge_results([sem, kw], n_results)
    elif mode == "visual":
        return visual_search(session, query, filters, n_results)
    elif mode == "caption":
        return caption_search(session, query, filters, n_results)
    elif mode == "multimodal":
        # Three-stream fusion: speech (transcript embeddings),
        # vision (CLIP image embeddings), description (VLM caption embeddings).
        # Each stream is optional — empty collections return [] and are skipped.
        sem = semantic_search(session, query, filters, n_results)
        vis = visual_search(session, query, filters, n_results)
        cap = caption_search(session, query, filters, n_results)
        return _merge_results([sem, vis, cap], n_results)
    else:
