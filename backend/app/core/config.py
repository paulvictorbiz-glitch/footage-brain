...
class Settings(BaseSettings):
    ...
    # ── VLM captioner (multimodal third stream) ───────────────────────────────
    # Enabled by default so the multimodal search button works out of the box.
    # The stage is lazy: model only loads on the first caption call. Files
    # already marked `captioned=True` are skipped, so flipping this on/off is
    # idempotent for previously processed files.
    captioner_enabled: bool = True
    captioner_model: str = "Salesforce/blip-image-captioning-large"
    captioner_device: str = "auto"   # "auto" | "cpu" | "cuda" | "mps"
    captioner_batch_size: int = 4
    captioner_max_new_tokens: int = 30

    # ── Vector store backend ──────────────────────────────────────────────────
    # "chroma"   — local SQLite-backed Chroma (default; zero setup).
    # "qdrant"   — embedded Qdrant under qdrant_path, or remote via qdrant_url.
    # "lancedb"  — embedded LanceDB under lancedb_path.
    vector_store: str = "chroma"
    qdrant_url: str = ""            # e.g. "http://localhost:6333" (overrides qdrant_path)
    qdrant_api_key: str = ""
    qdrant_path: str = ""           # local persistent dir (auto: <data_root>/qdrant)
    lancedb_path: str = ""          # local persistent dir (auto: <data_root>/lancedb)
    ...
    def _apply_data_root(cls, values: dict) -> dict:
        ...
        if not values.get("qdrant_path"):
            values["qdrant_path"] = str(base / "qdrant")
        if not values.get("lancedb_path"):
            values["lancedb_path"] = str(base / "lancedb")
