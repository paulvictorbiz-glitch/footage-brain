...
powershell -NoProfile -Command ^
  "try { Invoke-RestMethod -Method POST 'http://localhost:8765/api/dashboard/jobs/resume' | Out-Null; Write-Host '  [OK] Jobs unpaused.' } catch { Write-Host ('  [INFO] ' + $_.Exception.Message) }" 2>nul

REM ── Start / wake the worker ───────────────────────────────────────────────────

echo  Waking ingest worker...
powershell -NoProfile -Command ^
  "try { Invoke-RestMethod -Method POST 'http://localhost:8765/api/dashboard/start-worker' | Out-Null; Write-Host '  [OK] Worker running.' } catch { Write-Host ('  [WARN] ' + $_.Exception.Message) }" 2>nul
...
if /i "!DO_SCAN!"=="Y" (
    ...
    echo $r = Invoke-RestMethod -Method POST 'http://localhost:8765/api/sources/scan-all' > "%TEMP%\fb_scan.ps1"
...
echo $q = Invoke-RestMethod 'http://localhost:8765/api/dashboard/jobs' > "%TEMP%\fb_queue.ps1"
...
echo $sources = Invoke-RestMethod 'http://localhost:8765/api/sources' > "%TEMP%\fb_src.ps1"
...
echo  Monitor progress at http://localhost:8765 (Dashboard tab).
echo.
